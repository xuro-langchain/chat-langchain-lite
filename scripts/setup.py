...
def setup_dataset() -> str:
    ...
    """
    from evals.dataset import create_or_update_dataset, create_or_update_tool_adherence_dataset
    from langsmith import Client

    print(f"\n[2/4] Setting up dataset '{DATASET_NAME}'...")
    create_or_update_dataset()
    # Secondary "tool adherence" showcase dataset — not used by CI.
...
def seed_baseline_experiments() -> None:
    ...
    for model_id, label in _BASELINE_MODELS:
        ...
        prefix = f"baseline-{label}-chat-langchain-lite-demo-{user}"
